      ...
      <div className="flex border-b border-gray-800 overflow-x-auto scrollbar-none">
        {TABS.map((tab) => (
          <button
            ...
            className={cn(
              "px-3 sm:px-5 py-3 sm:py-4 text-xs sm:text-sm font-medium whitespace-nowrap transition-colors border-b-2 flex-shrink-0",
              activeTab === tab.id
        ...
        {activeTab === "overview" && (
          <div className="space-y-4 sm:space-y-6">
            <div>
              ...
              <p className="text-gray-400 leading-relaxed text-sm sm:text-base">{property.description}</p>
            </div>
        ...
        {activeTab === "finance" && (
          <div className="space-y-4 sm:space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              <div>
                ...
                <div className="bg-gray-800/50 rounded-lg overflow-hidden">
                  {[
                    ...
                    ["Gesamte Nebenkosten", formatCurrency(analysis.purchaseCostsTotal)],
                  ].map(([label, value], i) => (
                    <div key={label} className={cn("flex justify-between px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm", i === 5 ? "bg-gray-700/50 font-semibold text-white border-t border-gray-700" : "text-gray-400 border-b border-gray-700/50")}>
                      <span>{label}</span>
                      <span className={i === 5 ? "text-white" : "text-gray-300"}>{value}</span>
                    </div>
                  ))}
                  <div className="flex justify-between px-3 sm:px-4 py-2 sm:py-2.5 bg-blue-600/20 text-xs sm:text-sm font-bold text-blue-400">
              ...
              <div>
                <h3 className="text-white font-semibold mb-3 text-sm sm:text-base">Cashflow (20% EK, 3,8% Zinsen)</h3>
                <div className="bg-gray-800/50 rounded-lg overflow-hidden">
                  {[
                    ...
                    <div key={label} className={cn("flex justify-between px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm", i === 2 ? "bg-gray-700/30 border-y border-gray-700/50" : "border-b border-gray-700/30 text-gray-400")}>
                      <span>{label}</span>
                      <span className="text-gray-300 font-medium">{value}</span>
                    </div>
                  ))}
                  <div className={cn("flex justify-between px-3 sm:px-4 py-2 sm:py-2.5 font-bold text-xs sm:text-sm", analysis.cashflow >= 0 ? "bg-green-600/20 text-green-400" : "bg-red-600/20 text-red-400")}>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3">
                {[
                  ...
                  <div key={item.label} className="bg-gray-800/50 rounded-lg p-3 sm:p-4 text-center">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div key={item.label} className="flex items-center justify-between bg-gray-800/50 rounded-lg px-3 sm:px-4 py-2.5">
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
              {[
                ...
                <div key={scenario.key} className={cn("border rounded-xl p-3 sm:p-4", scenario.bg)}>
        ...
        {activeTab === "report" && (
          <div className="space-y-4 sm:space-y-6">
            ...
            <div className="bg-gray-800/50 rounded-xl p-4 sm:p-5 border border-gray-700/50">
              <p className="text-gray-300 leading-relaxed text-sm sm:text-base">{analysis.aiReport}</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-3 sm:p-4">
                ...
                <ul className="space-y-2 text-gray-400 text-sm">
                  ...
                  {analysis.grossYield > 5 && <li>• Überdurchschnittliche Rendite von {formatPercent(analysis.grossYield)}</li>}
            ...
            <div className={cn("border rounded-xl p-4 sm:p-5 flex items-center gap-4",
              analysis.dealScore >= 80 ? "bg-green-500/10 border-green-500/20" :
              analysis.dealScore >= 70 ? "bg-blue-500/10 border-blue-500/20" :
              "bg-gray-800/50 border-gray-700/50"
            )}>
              <div className="text-4xl font-bold flex-shrink-0" style={{ color: analysis.dealScore >= 80 ? "#4ade80" : analysis.dealScore >= 70 ? "#60a5fa" : "#9ca3af" }}>
                {analysis.dealScore}
              </div>
              <div>
