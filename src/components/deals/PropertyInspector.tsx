...
function RisikoBar({ score }: { score: number }) {
  const color = score >= 70 ? "bg-red-500" : score >= 50 ? "bg-orange-500" : score >= 30 ? "bg-amber-500" : "bg-green-500";
  const label = score >= 70 ? "Kritisch" : score >= 50 ? "Hoch" : score >= 30 ? "Mittel" : "Gering";
  return (
    <div>
      <div className="flex justify-between text-xs text-gray-500 mb-1">
        <span>Sonderumlage-Risiko</span>
        <span className={cn("font-bold", score >= 70 ? "text-red-400" : score >= 50 ? "text-orange-400" : score >= 30 ? "text-amber-400" : "text-green-400")}>{label} ({score}/100)</span>
      </div>
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Gesamtkosten (Min.)", value: formatCurrency(analyse.gesamtkostenMin), sub: `bis ${formatCurrency(analyse.gesamtkostenMax)}`, color: "text-white" },
          { label: "KfW-Förderung (max.)", value: formatCurrency(analyse.foerderungMax), sub: "Zuschüsse", color: "text-green-400" },
          { label: "Nettokosten nach Förderung", value: formatCurrency(analyse.nettokosten), sub: "realistisch", color: "text-amber-400" },
          { label: "Mieterhöhung möglich", value: `+${formatCurrency(analyse.monatlicheUmlage)}/Mo`, sub: "§559 BGB Umlage", color: "text-blue-400" },
        ].map((card) => (
        ...
        {analyse.massnahmen.map((m, i) => (
          <div key={i} className={cn(
            ...
            <div className="flex items-start justify-between gap-3 mb-2">
              ...
              <p className="text-white font-bold text-sm whitespace-nowrap tabular-nums">{formatCurrency(m.kostenMin)}–{formatCurrency(m.kostenMax)}</p>
            </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-5 space-y-3">
        ...
        {[
          { label: "Aktuelle Miete", value: formatCurrency(analyse.aktuelleKaltmiete), width: barWidth, color: "bg-white" },
          { label: "Marktmiete (Mietspiegel)", value: formatCurrency(analyse.marktmieteSchaetzung), width: marktBarWidth, color: "bg-blue-500" },
          { label: "Mietpreisbremse Max.", value: formatCurrency(analyse.mietpreisbremseMax), width: maxBarWidth, color: "bg-amber-500" },
        ].map((bar) => (
          <div key={bar.label}>
            <div className="flex justify-between text-xs text-gray-500 mb-1"><span>{bar.label}</span><span>{bar.value}</span></div>
        ...
        {sanierung.monatlicheUmlage > 0 && (
          <div>
            ...
            <div className="h-3 bg-gray-700 rounded-full overflow-hidden">
              <div className="h-full bg-green-500 rounded-full" style={{ width: `${Math.min(100, (analyse.nachSanierungMax / (analyse.mietpreisbremseMax * 1.1)) * 100)}%` }} />
            </div>
          </div>
        )}
      </div>
      <div className={cn("flex items-start gap-3 border rounded-xl p-4", bewertungColor)}>
        ...
        <div>
          <p className="font-semibold text-sm mb-1">Miete {analyse.bewertung}{analyse.potenzialProzent > 0 && ` – ${analyse.potenzialProzent}% Potenzial`}</p>
...
function WEGRisikoSection({ property }: { property: Property }) {
  ...
  const risikoColor = { gering: "text-green-400", mittel: "text-amber-400", hoch: "text-orange-400", kritisch: "text-red-400" }[analyse.risikoLevel];
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {analyse.redFlags.length > 0 && (
          <div className="bg-red-500/5 border border-red-500/20 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3"><AlertTriangle className="w-4 h-4 text-red-400" /><h4 className="text-red-400 font-semibold text-sm">Risikofaktoren</h4></div>
            <ul className="space-y-2">{analyse.redFlags.map((flag, i) => <li key={i} className="text-gray-400 text-sm flex items-start gap-2"><span className="text-red-500 mt-0.5 flex-shrink-0">·</span>{flag}</li>)}</ul>
          </div>
        )}
        {analyse.grueneFlags.length > 0 && (
          <div className="bg-green-500/5 border border-green-500/20 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3"><CheckCircle className="w-4 h-4 text-green-400" /><h4 className="text-green-400 font-semibold text-sm">Positive Faktoren</h4></div>
            <ul className="space-y-2">{analyse.grueneFlags.map((flag, i) => <li key={i} className="text-gray-400 text-sm flex items-start gap-2"><span className="text-green-500 mt-0.5 flex-shrink-0">·</span>{flag}</li>)}</ul>
          </div>
        )}
      </div>
      <div className="flex items-start gap-3 bg-blue-500/5 border border-blue-500/20 rounded-xl p-4">
        ...
        <div>
          <p className={cn("font-semibold text-sm mb-1", risikoColor)}>WEG-Risiko: {analyse.risikoLevel.charAt(0).toUpperCase() + analyse.risikoLevel.slice(1)}</p>
          <p className="text-gray-400 text-sm">{analyse.handlungsempfehlung}</p>
        </div>
      </div>
      <div className="bg-gray-800/30 border border-gray-700/50 rounded-xl p-4">
        <h4 className="text-white text-sm font-semibold mb-3 flex items-center gap-2"><Shield className="w-4 h-4 text-blue-400" /> Due Diligence Checkliste WEG</h4>
        <ul className="space-y-2">
          {["WEG-Protokolle der letzten 3 Jahre anfordern", "Aktuelle Instandhaltungsrücklage in € beim Verwalter erfragen", "Auf beschlossene oder geplante Sonderumlagen prüfen", "Teilungserklärung und Gemeinschaftsordnung prüfen", "Wirtschaftsplan des laufenden Jahres anfordern", "Gebäudeversicherung und Deckungssummen prüfen", analyse.risikoLevel !== "gering" ? "Bausachverständigen für Begehung beauftragen" : null].filter(Boolean).map((item, i) => (
            <li key={i} className="flex items-start gap-2 text-sm text-gray-400"><span className="w-4 h-4 border border-gray-600 rounded flex-shrink-0 mt-0.5" />{item}</li>
          ))}
        </ul>
      </div>
    </div>
    ...
    <div className="space-y-5">
      <div className="flex gap-2 flex-wrap">
        {SUBTABS.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => onSubTabChange(id)} className={cn(
            "flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium border transition-all",
            activeSubTab === id ? "bg-blue-600/15 border-blue-500/40 text-blue-400" : "bg-gray-800/50 border-gray-700/50 text-gray-400 hover:text-white hover:border-gray-600"
          )}>
