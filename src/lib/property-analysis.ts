...
export interface Massnahme {
  ...
  kostenMin: number;
  kostenMax: number;
  kfwFoerderung: number; // Prozent
  kfwProgramm: string;
  mietsteiggerungProJahr: number; // € nach Modernisierungsumlage
  zeitaufwandWochen: number;
  energiegewinn: number; // Energieklassen-Verbesserung (0 = keine)
}
...
export interface SanierungsAnalyse {
  ...
  nettokosten: number;
  monatlicheUmlage: number; // §559 BGB: 8% der Modernisierungskosten p.a. / 12
...
const ENERGY_SCORE: Record<string, number> = {
  "A++": 1, "A+": 2, A: 3, B: 4, C: 5, D: 6, E: 7, F: 8, G: 9, H: 10,
};
const ENERGY_FROM_SCORE: Record<number, string> = {
  1: "A++", 2: "A+", 3: "A", 4: "B", 5: "C", 6: "D", 7: "E", 8: "F", 9: "G", 10: "H",
};
...
export function analysiereSanierung(p: Property): SanierungsAnalyse {
  ...
  if (isOelGas && !isFernwaerme) {
    ...
    massnahmen.push({
      titel: "Heizungsanlage modernisieren",
      beschreibung: pflicht
        ? `${p.heatingType} muss gemäß GEG §72 ersetzt werden (Baujahr vor 1980 oder Ölheizung). Wärmepumpe oder Fernwärme empfohlen.`
        : `${p.heatingType} nähert sich dem Ende der Nutzungsdauer. Tausch auf Wärmepumpe senkt Betriebskosten und verbessert Energieklasse.`,
      status: pflicht ? "pflicht" : "empfohlen",
      prioritaet: pflicht ? "hoch" : "mittel",
      kostenMin,
      kostenMax,
      kfwFoerderung: 35,
      kfwProgramm: "BEG EM (Nr. 458) – bis 35% Zuschuss",
      mietsteiggerungProJahr: Math.round((kostenMin * 0.08)),
      zeitaufwandWochen: 3,
      energiegewinn: 2,
    });
  }
  ...
  if (p.yearBuilt < 2002 && eScore >= 5) {
    ...
    massnahmen.push({
      titel: "Dachdämmung / oberste Geschossdecke",
      beschreibung: `Gebäude vor 2002 sind gemäß GEG §47 nachrüstpflichtig, sofern nicht bereits gedämmt. Dachdämmung reduziert Heizkosten um 15–25%.`,
      status: p.yearBuilt < 1995 ? "pflicht" : "empfohlen",
      prioritaet: "mittel",
      kostenMin,
      kostenMax,
      kfwFoerderung: 15,
      kfwProgramm: "BEG EM (Nr. 458) – Einzelmaßnahme Dämmsystem",
      mietsteiggerungProJahr: Math.round(kostenMin * 0.08),
      zeitaufwandWochen: 2,
      energiegewinn: 1,
    });
  ...
  if (p.yearBuilt < 1990) {
    ...
    massnahmen.push({
      ...
      beschreibung: `Einfach- oder zweifachverglaste Fenster (typisch vor 1990) verursachen erhebliche Wärmeverluste. Dreifachverglasung senkt Heizkosten, erhöht Wohnkomfort und Vermietbarkeit.`,
      status: eScore >= 7 ? "empfohlen" : "optional",
      prioritaet: eScore >= 7 ? "mittel" : "niedrig",
      kostenMin,
      kostenMax,
      kfwFoerderung: 15,
      kfwProgramm: "BEG EM (Nr. 458) – Fenster und Außentüren",
      mietsteiggerungProJahr: Math.round(kostenMin * 0.08),
      zeitaufwandWochen: 1,
      energiegewinn: 1,
    });
  ...
  if (p.yearBuilt < 1970) {
    massnahmen.push({
      titel: "Elektroinstallation erneuern",
      beschreibung: `Elektroanlagen vor 1970 entsprechen oft nicht mehr dem Stand der Technik und können Brandgefahr darstellen. Pflichtprüfung beim Kauf empfohlen.`,
      status: "pflicht",
      prioritaet: "hoch",
      kostenMin: Math.round(area * 60),
      kostenMax: Math.round(area * 100),
      kfwFoerderung: 0,
      kfwProgramm: "Keine direkte KfW-Förderung",
      mietsteiggerungProJahr: 0,
      zeitaufwandWochen: 2,
      energiegewinn: 0,
    });
  }
  ...
  if (p.yearBuilt < 1985) {
    massnahmen.push({
      titel: "Badezimmer modernisieren",
      beschreibung: `Veraltete Bäder reduzieren Vermietbarkeit und erzielbare Miete erheblich. Modernisierung ermöglicht Mieterhöhung nach §559 BGB.`,
      status: "empfohlen",
      prioritaet: age > 40 ? "mittel" : "niedrig",
      kostenMin: 7000,
      kostenMax: 14000,
      kfwFoerderung: 0,
      kfwProgramm: "Ggf. KfW Nr. 159 (Altersgerecht Umbauen) bei barrierefreiheit",
      mietsteiggerungProJahr: Math.round(7000 * 0.08),
      zeitaufwandWochen: 3,
      energiegewinn: 0,
    });
  }

  // 6. FASSADE (nur bei Haus oder schlechter Energieklasse)
  if (eScore >= 7 && p.type !== "APARTMENT") {
    ...
    massnahmen.push({
      titel: "Fassadendämmung (WDVS)",
      beschreibung: `Außendämmung ist bei Energieklasse ${p.energyClass} der effektivste Weg zur Energieklassenverbesserung. Senkt Heizkosten drastisch und erhöht Marktwert.`,
      status: "empfohlen",
      prioritaet: "mittel",
      kostenMin,
      kostenMax,
      kfwFoerderung: 20,
      kfwProgramm: "BEG EM (Nr. 458) – Wärmedämmung Außenwand",
      mietsteiggerungProJahr: Math.round(kostenMin * 0.08),
      zeitaufwandWochen: 4,
      energiegewinn: 2,
    });
  ...
  const sanierungsrisiko: SanierungsAnalyse["sanierungsrisiko"] =
    pflichtCount >= 3 ? "sehr hoch" :
    pflichtCount >= 2 ? "hoch" :
    pflichtCount >= 1 ? "mittel" : "gering";
  ...
  return {
    ...
    nettokosten,
    monatlicheUmlage,
    energieklasseNachSanierung,
    gesamtZeitaufwandWochen,
    sanierungsrisiko,
  };
}
...
// Grobe Mietspiegel-Referenzwerte (€/m²) nach Stadt
...
export function analysiereMietpotenzial(p: Property, sanierung: SanierungsAnalyse): MietpotenzialAnalyse {
  ...
  // Nach Sanierung: Modernisierungsumlage nach §559 BGB
  ...
  const bewertung: MietpotenzialAnalyse["bewertung"] =
    p.monthlyRent < marktmieteSchaetzung * 0.95 ? "unter Markt" :
    p.monthlyRent > marktmieteSchaetzung * 1.05 ? "über Markt" : "am Markt";

  const handlungsempfehlung =
    bewertung === "unter Markt"
      ? `Miete liegt ${Math.abs(potenzialProzent)}% unter Marktniveau. Stufenweise Anpassung bis zur Mietpreisbremsen-Grenze (${Math.round(marktmieteProSqm * 1.1 * 100) / 100} €/m²) möglich.`
      : bewertung === "am Markt"
      ? `Miete entspricht dem Markt. Steigerungen nur durch Modernisierungsmaßnahmen (§559 BGB) oder bei Mieterwechsel realisierbar.`
      : `Miete liegt über Marktniveau. Bei Mieterwechsel Risiko der Absenkung beachten.`;

  return {
    aktuelleKaltmiete: p.monthlyRent,
    marktmieteSchaetzung,
    mietpreisbremseMax,
    potenzialProzent,
    nachSanierungMin,
    nachSanierungMax,
    modernisierungsumlagePotenzial,
    bewertung,
    handlungsempfehlung,
    mietpreisProSqm,
    marktpreisProSqm: marktmieteProSqm,
  };
}
...
export interface WEGRisikoAnalyse {
  ...
  sonderumlagRisikoScore: number; // 0-100
...
export function analysiereWEGRisiko(p: Property): WEGRisikoAnalyse {
  ...
  // Empfehlung: 0,8–1,5% des Gebäudewerts p.a. als Rücklage, vereinfacht ~1% Kaufpreis
  const empfohlenesHausgeld = Math.round((p.price * 0.01 / 12) + 150); // Rücklage + Verwaltung + Versicherung
  const empfohlenProSqm = Math.round((empfohlenesHausgeld / p.area) * 100) / 100;

  // Jährliche Rücklage: ca. 8–14€/m² je nach Alter
  ...
  // Bewertung
  if (hausgeldProSqm < 2.0) {
    redFlags.push(`Hausgeld sehr niedrig (${hausgeldProSqm} €/m²) – Rücklagen wahrscheinlich unzureichend`);
  } else if (hausgeldProSqm < 2.8) {
    redFlags.push(`Hausgeld knapp (${hausgeldProSqm} €/m²) – WEG-Protokoll auf Sonderumlagen prüfen`);
  } else {
    grueneFlags.push(`Angemessenes Hausgeld (${hausgeldProSqm} €/m²)`);
  }
  ...
  if (aktuelleRuecklageSchaetzung < jaehrlicheRuecklageEmpfehlung * 0.5) {
    redFlags.push("Geschätzte Rücklage deutlich unter Empfehlung – Sonderumlage wahrscheinlich");
  }

  if (p.hausgeld > empfohlenesHausgeld) grueneFlags.push("Hausgeld über Empfehlung – solide Rücklagenbildung wahrscheinlich");

  const ruecklagenEinschaetzung: WEGRisikoAnalyse["ruecklagenEinschaetzung"] =
    hausgeldProSqm < 2.0 ? "zu niedrig" :
    hausgeldProSqm < 2.8 ? "knapp" :
    hausgeldProSqm < 4.0 ? "ausreichend" : "gut";

  // Risiko-Score: höher = schlechter
  let score = 0;
  ...
  const risikoLevel: WEGRisikoAnalyse["risikoLevel"] =
    score >= 70 ? "kritisch" : score >= 50 ? "hoch" : score >= 30 ? "mittel" : "gering";

  const handlungsempfehlung =
    risikoLevel === "kritisch"
      ? "Unbedingt alle WEG-Protokolle der letzten 3 Jahre anfordern und auf beschlossene oder geplante Sonderumlagen prüfen. Instandhaltungsrücklage beim Verwalter erfragen."
      : risikoLevel === "hoch"
      ? "WEG-Protokolle der letzten 2 Jahre prüfen. Insbesondere auf Beschlüsse zu Dacherneuerung, Leitungssanierung oder Fahrstuhl achten."
      : risikoLevel === "mittel"
      ? "WEG-Protokoll des letzten Jahres anfordern. Aktuelle Instandhaltungsrücklage in EUR beim Verwalter erfragen."
      : "Solides WEG-Umfeld. Aktuelles WEG-Protokoll und Rücklagenspiegel als Standardprüfung anfordern.";

  return {
    hausgeldProSqm,
    empfohlenesHausgeld,
    ruecklagenEinschaetzung,
    sonderumlagRisikoScore: score,
    risikoLevel,
    redFlags,
    grueneFlags,
    handlungsempfehlung,
    jaehrlicheRuecklageEmpfehlung,
    aktuelleRuecklageSchaetzung,
  };
}
